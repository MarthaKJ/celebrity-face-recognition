...
def main(args):
    ...
    correct_predictions_am = AverageMeter()
    total_samples_am = AverageMeter()
    ...
    for epoch in range(start_epoch, cfg.num_epoch):
        ...
        for _, (img, local_labels) in enumerate(train_loader):
            ...
            # Calculate accuracy
            _, predicted = torch.max(local_embeddings, 1)
            correct_predictions_am.update((predicted == local_labels).sum().item(), local_labels.size(0))
            total_samples_am.update(local_labels.size(0), 1)
            ...
            with torch.no_grad():
                accuracy = (correct_predictions_am.avg / total_samples_am.avg) * 100
                if wandb_logger:
                    wandb_logger.log(
                        {
        ...
        # After each epoch, log the accuracy
        epoch_accuracy = (correct_predictions_am.avg / total_samples_am.avg) * 100
        logging.info(f"Epoch {epoch + 1}/{cfg.num_epoch}, Accuracy: {epoch_accuracy:.2f}%")
