...
import sys
import time
from utils import losses  # Import the losses module from utils
...
def train(args):
    ...
    if args.use_synthetic_dataset:
        ...
        trainset = CommonDataset(
            root_dir=args.data_dir, label_file=args.label_file, fp16=args.fp16, is_bin=args.is_bin
        )
    ...
    if rank == 0:
        ...
        logging.info(f"total_epoch: {total_epoch}")
        logging.info(f"decay_steps: {decay_steps}")

    base_lr = total_batch_size * args.lr / 512
    lr_scheduler = paddle.optimizer.lr.PiecewiseDecay(
        boundaries=decay_steps, values=[base_lr * (args.lr_decay**i) for i in range(len(decay_steps) + 1)]
    )
    if warmup_steps > 0:
        lr_scheduler = paddle.optimizer.lr.LinearWarmup(lr_scheduler, warmup_steps, 0, base_lr)
    ...
    callback_logging = CallBackLogging(args.log_interval_step, rank, world_size, total_steps, args.batch_size, writer)
    ...
    train_loader = paddle.io.DataLoader(
        ...
        batch_sampler=paddle.io.DistributedBatchSampler(
            dataset=trainset, batch_size=args.batch_size, shuffle=True, drop_last=True
        ),
    )
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        for step, data in enumerate(train_loader):
            ...
            if global_step >= total_steps:
                break
            sys.stdout.flush()
            if rank == 0 and global_step > 0 and global_step % args.log_interval_step == 0:
                train_reader_cost = 0.0
                train_run_cost = 0.0
                total_samples = 0
            reader_start = time.time()
