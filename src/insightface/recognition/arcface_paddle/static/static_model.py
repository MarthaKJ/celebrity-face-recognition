...
from .utils.optimization_pass import amp_pass, gather_optimization_pass
    ...
    def __init__(
        ...
        if self.mode == "train":
            ...
            with paddle.static.program_guard(self.main_program, self.startup_program):
                with paddle.utils.unique_name.guard():
                    self.backbone = eval(f"backbones.{self.backbone_class_name}")(
                        num_features=self.embedding_size, is_train=True, fp16=self.fp16, dropout=dropout
                    )
            ...
            with paddle.static.program_guard(self.main_program, self.startup_program):
                with paddle.utils.unique_name.guard():
                    self.backbone = eval(f"backbones.{self.backbone_class_name}")(
                        num_features=self.embedding_size, is_train=False, fp16=self.fp16, dropout=dropout
                    )
                    assert "feature" in self.backbone.output_dict
